...
func (b *BuildOptions) BuildImage() {
	...
	if b.Hypervisor == VSphere {
		if b.VsphereConfig.LinkedClone == "true" && b.VsphereConfig.Template == "" {
			log.Fatalf("Error: When linked_clone is set to true, template cannot be empty")
		}
		...
		switch b.Os {
		case Ubuntu:
			if b.VsphereConfig.LinkedClone == "true" {
				if b.Firmware == EFI {
